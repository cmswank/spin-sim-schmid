...
Grad=1E-7/100;%1E-7;%1E-5/100;  %first number is G/cm, divide by 100 to make it T/m
Efield=7.5E6; %V/m
...
ghe3=sqrt(pi.*b/2)./q.*faddeeva(1i.*sqrt(b/2).*(lam+1i.*w)./q,1000);  %the G function  
...
dfm=203789000^2*Efield/3E8^2*Grad*(2*pi*fw.*imag(Sxtheory)+1/12*Lx^2)/2/pi;

%E-B relaxation in Hz!!!!!
Rfm=2*203789000^2*Efield/3E8^2*Grad*(2*pi*fw.*real(Sxtheory));

%B squared phase in Hz!!!!!
dwB2he=203789000^2/2*Grad^2*imag(Sxtheory)/2/pi; %in Hz
