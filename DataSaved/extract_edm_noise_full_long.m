%Code to extract the new setup Only w=6000, for all  noise analysis
%for all assortments of directions E and Gsd including 0 for each
%Gsd is the fast moving variable, Gsd=-G,0,G. (change every number)
%then E= -E 0 E then (change every 3)
%spin type 3he, then neutrons. (change every 9.)  
%therefore 18 is in a set. 
% run 4300 E=7.5E6kV/m G=1E-3 B1/m, B1 is for 6000 rad/s xc. 
% run 4320 E=7.5E6kV/m G=5E-4 B1/m, B1 is for 6000 rad/s xc. 
% run 4340 starting phase = .8 rad or so, E=7.5E6kV/m G=1E-3 B1/m, B1 is for 6000 rad/s xc.
% run 4360 same as 4300 with smoothed pulsed modulation. 

%%%%%%%%%%Table of combinations. 
%Script flattens to the he3 definiton for theta.  (via the adder variable)
%3he
% n=1 G=+, E=-,
% n=2 G=0, E=-,
% n=3 G=-, E=-,
...
% n=17 G=0, E=-,
% n=18 G=-, E=-,
% frequency from edm -> DeltaOmega=dn * 4 * E/hbar,  
%= dn(ecm) * 4 * E (V/cm)/6.58212E-16 (e*V*s) 
%= for dn=1E-25 and E = 75kV/cm we have
% 4.559E-5 rad/s

adder=15; %adder needs to start at 15 for this spagetti code to work
...
% the following are matrices (time (Bin+1), particles (Event-1))
    ...
    t = squeeze(tlarge(:,1));
%    error('you are a moron, fuck you');
    phi_up=angle(sy1+1i*sz1);
    
    datafile = strcat('/data1/cmswank/spin-sim-xliu/ExtractData/SpinDressingCrossTerm_',num2str(start+(n-1)+adder),'.dat');
...
%     
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%55
%     
%     datafile = strcat('/data1/cmswank/spin-sim-xliu/ExtractData/SpinDressingCrossTerm_',num2str(start+(n-1)+1),'.dat');
...
%  
%     datafile = strcat('/data1/cmswank/spin-sim-xliu/ExtractData/SpinDressingCrossTerm_',num2str(start+(n-1)+10),'.dat');
...
%     
%     
%     [T2n,T2uppern,T2lowern] = sparseT2fit(t,abs(Sz2+1i*Sy2)/Event,Event);
%     [T2,T2upper,T2lower] = sparseT2fit(t,abs(Sz1+1i*Sy1)/Event,Event);
%     B0hat=[1,0,0];
%     sphi1=ExtractPhases(sx1,sy1,sz1,sx2,sy2,sz2,B0hat);
%     sphisave(:,n)=sphi1;
%     %sphi2=ExtractPhases(sx2,sy2,sz2,sx4,sy4,sz4,B0hat);
%     
  %figure(n+22)
...
%     [theta1] = getTheta(sx1,sy1,sz1,sx2,sy2,sz2);
%     [theta2] = getTheta(sx3,sy3,sz3,sx4,sy4,sz4);
%    %why are these two sooooo different!!!
%     thetamean=mean(theta,2);
    %sndotsn=sndots3Mean(sx2,sy2,sz2,sx4,sy4,sz4);
    %s3dots3=sndots3Mean(sx1,sy1,sz1,sx3,sy3,sz3);
    %sndots3=sndots3long(flip(sx1,2),flip(sy1,2),flip(sz1,2),sx2,sy2,sz2);
...
%     costheta=sndots3./(S1.*S2);
    ...
    %eval(['Signallongl',num2str(n),'=1-sndots3;']);
    eval(['Signallong',num2str(start+n),'=1-sndots3;']);
    eval(['n3theta',num2str(start+n),'=acos(sndots3);']);
    %hold off 
    % plot(t,(signal));
%     hold on
%     plot(t,signalEd);
...
%     
     ...
     % and seeing this is somthing that is trivial 
...
SignalEDM=wdn.*t;
%
% SvxE2lp=Signallp(:,2)-Signallp(:,5);
% SvxEGlp=Signallp(:,1)-Signallp(:,4)-SvxE2lp;
% SG2lp=Signallp(:,7)-Signallp(:,9);
% 
% 
% SvxE2sp=Signalsp(:,2)-Signalsp(:,5);
% SvxEGsp=Signalsp(:,1)-Signalsp(:,4)-SvxE2sp;
% SG2sp=Signalsp(:,7)-Signalsp(:,9);
% 
