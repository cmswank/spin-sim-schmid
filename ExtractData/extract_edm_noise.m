...
for n=1:k
    datafile = strcat('/data1/cmswank/spin-sim-xliu/ExtractData/SpinDressingCrossTerm_',num2str(start+(n-1)*4),'.dat');
...
% the following are matrices (time (Bin+1), particles (Event-1))
    ...
    datafile = strcat('/data1/cmswank/spin-sim-xliu/ExtractData/SpinDressingCrossTerm_',num2str(start+(n-1)*4+1),'.dat');
...
%     tlarge = squeeze(B(10,:,2:Event)); 
    ...
    B0hat=[1,0,0];
    sphi1=ExtractPhases(sx1,sy1,sz1,sx3,sy3,sz3,B0hat);
