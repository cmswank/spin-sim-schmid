function [T2,fwsd,T2McgregorRf,lam,I2]= ThermalCollisionRelaxJ0(wdress,B1,gamma,B0,Temp,Grad,Lx)
...
phe3=(phe3p+phe3m);%don't use the other ones they are zero!  %transform of cos(wt) 
...
%R2McGregorRF=w0f.^2.*Lx^4*gamma^2*(Grad)^2/2/120/D3.
...
%T1
w0f=gammap/gammaG;
R2=w0f.^2.*gamma^2*Grad^2./4.*real(Sxtheorysd);
I2=w0f.^2.*gamma^2*Grad^2./4.*imag(Sxtheorysd);
%disp(Grad);
...
R2McGregorRF=w0f.^2.*Lx^4*gamma^2*(Grad)^2/4/120/D3.*(1./(1+wm.^2.*Lx.^4./D3.^2./pi.^4)+1./(1+wp.^2.*Lx.^4./D3.^2./pi.^4));
